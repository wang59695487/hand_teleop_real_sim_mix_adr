...
class Record3DSingleHandMotionControl:
    ...
    def init_step(self):
        ...
        if np.linalg.norm(offset - self.previous_offset[self.hand_mode]) > 0.05 and has_init_offset:
            ...
            if self.hand_mode == 'left_hand':
                self.hand_mocap.mean_pose = self.hand_mocap.left_hand_mean_pose
            else:
                self.hand_mocap.mean_pose = self.hand_mocap.right_hand_mean_pose
    ...
    def compute_hand_zero_pos(self):
        ...
        if self.hand_mode == "right_hand":
            ...
            hand_index = [20] + list(range(25, 40)) + list(range(66, 71))  # 20 for left wrist. 20 finger joints
        ...
        with torch.no_grad():
            if self.hand_mode == "right_hand":
                ...
                output = smplx(body_pose=body_pose, left_hand_pose=-smplx.left_hand_mean, betas=shape_params,
                           return_verts=True)                
    ...
    def mean_hand_pose(self):
        if self.hand_mode == "right_hand":
            ...
            return self.hand_mocap.model.smplx.left_hand_mean.cpu().numpy()