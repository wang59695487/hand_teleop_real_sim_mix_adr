            ...
            for _, file_name in enumerate(demo_files):
                ...
                if args['task_name'] == 'pick_place':
                    ...
                    if np.fabs(x2) <= 0.01 and np.fabs(y2) <= 0.01:
                        continue
                    init_pose_aug_plate = sapien.Pose(
                        [x2, y2, 0], [1, 0, 0, 0])
...
def adr(args, current_rank, adr_dict):
    ...
    if current_rank == 1:
        ...
        meta_data_path = f"{args['sim_aug_dataset_folder']}/meta_data.pickle"
        if current_rank == 2:
            adr_dict['var_adr_light'] = adr_dict['var_adr_light'] + \
                0.2 if adr_dict['is_var_adr'] else adr_dict['var_adr_light']
            if adr_dict['var_adr_light'] > 2:
                adr_dict['var_adr_light'] = 2
                current_rank += 1

        elif current_rank == 3 and args['task_name'] == 'pick_place':
            adr_dict['var_adr_plate'] = adr_dict['var_adr_plate'] + \
                0.02 if adr_dict['is_var_adr'] else adr_dict['var_adr_plate']
            if adr_dict['var_adr_plate'] > 0.1:
                adr_dict['var_adr_plate'] = 0.1
                current_rank += 1

        elif current_rank == 4 and args['task_name'] == 'pick_place':
            adr_dict['var_adr_object'] = adr_dict['var_adr_object'] + \
                0.02 if adr_dict['is_var_adr'] else adr_dict['var_adr_object']
