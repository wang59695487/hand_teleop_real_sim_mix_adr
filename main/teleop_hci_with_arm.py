...
from hand_teleop.teleop.teleop_gui import GUIBase, DEFAULT_TABLE_TOP_CAMERAS,DEFAULT_ROTATING_CAMERAS
...
def main():
    ...
    if robot_name == "mano":
        ...
        qpos = [0, (-45/180)*np.pi, 0, 0, (45/180)*np.pi, (-90/180)*np.pi] + [0] * 16
        if task_name == "dclaw":
            qpos = [0, (20/180)*np.pi, -(85/180)*np.pi, 0, (112/180)*np.pi, -np.pi / 2] + [0] * 16
        qpos = np.array(qpos)
        robot.set_qpos(qpos)
        ...
        if "free" in robot_name:
            ...
            robot.set_pose(sapien.Pose(np.array([-0.55, 0, 0.00855]), transforms3d.euler.euler2quat(0, 0, 0)))
