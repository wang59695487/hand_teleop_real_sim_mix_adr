...
def compute_loss(agent, bc_train_dataloader, L, epoch):
    ...
    loss_dict = agent.compute_loss(
        obs.cuda(), robot_qpos.cuda(), action.cuda(), is_pad.cuda(), label.cuda())
...
def train_real_sim_in_one_epoch(agent, sim_real_ratio, it_per_epoch_real, it_per_epoch_sim, bc_train_dataloader_real,
    ...
    for _ in tqdm(range(it_per_epoch_sim)):
        ...
        l1_loss, kl_loss, act_loss = compute_loss(
            agent, bc_validation_dataloader_sim, L, epoch)
    ...
    for _ in tqdm(range(it_per_epoch_real)):
        ...
        l1_loss, kl_loss, act_loss = compute_loss(
            agent, bc_validation_dataloader_real, L, epoch)
        ...
        if not args["eval_only"]:
            ...
            if Prepared_Data['data_type'] == "real":
                log_dir = os.path.join(
                    "logs", f"{args['real_dataset_folder'].split('/')[-1]}_{Prepared_Data['data_type']}_{args['backbone_type']}_{cur_time}")
            else:
                log_dir = os.path.join(
                    "logs", f"{args['sim_dataset_folder'].split('/')[-1]}_{Prepared_Data['data_type']}_{args['backbone_type']}_{cur_time}")
            wandb.init(
                project="hand-teleop",
            ...
            for epoch in range(args['num_epochs']):
                ...
                if Prepared_Data['data_type'] == "real_sim":
                    ...
                    loss_real_sim = train_real_sim_in_one_epoch(agent, sim_real_ratio,
                                                                ...
                                                                Prepared_Data['bc_train_dataloader_real'], Prepared_Data[
                                                                    'bc_validation_dataloader_real'],
                ...
                if (epoch + 1) % args["eval_freq"] == 0 and (epoch+1) >= args["eval_start_epoch"]:
                    ...
                    if Prepared_Data['data_type'] == "sim":
                        with torch.inference_mode():
                            agent.policy.eval()
                            avg_success = eval_in_env(
                                args, agent, log_dir, epoch + 1, 4, 5)
            ...
            if Prepared_Data['data_type'] == "sim":
                ...
                with torch.inference_mode():
                    agent.policy.eval()
                    final_success = eval_in_env(
                        args, agent, log_dir, "best", 10, 10)
            ...
            path = args["ckpt"].split(
                "/")[-2]+"_eval_randomness_rank_"+str(args["randomness_rank"])
