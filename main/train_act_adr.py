...
from dataset.act_dataset import (
    argument_dependecy_checker,
    prepare_sim_aug_data,
    prepare_real_data,
    set_seed,
)
...
def train_and_aug(args, demo_files, log_dir, current_rank):
    ...
    # Prepare the data
    Prepared_Data_Sim = prepare_sim_aug_data(
        sim_dataset_folder=args["sim_dataset_folder"],
        sim_dataset_aug_folder=sim_aug_path,
        sim_aug_demo_length=sim_aug_demo_length,
        sim_batch_size=args["sim_batch_size"],
        val_ratio=args["val_ratio"],
        seed=20230920,
        chunk_size=args["num_queries"],
    )
    ...
    print("---------------------")
    print(
        "Concatenated Observation (State + Visual Obs) Shape: {}".format(
    ...
    for epoch in range(epochs):
        print("  ", "Epoch: ", epoch)
        agent.policy.train()
        loss_train_dict_sim = train_in_one_epoch(
            agent,
            Prepared_Data_Sim["it_per_epoch"],
            Prepared_Data_Sim["bc_train_dataloader"],
            Prepared_Data_Sim["bc_validation_dataloader"],
            L,
            epoch,
        )
        metrics = {
        ...
        if args["real_dataset_folder"] is not None:
            loss_train_dict_real = train_in_one_epoch(
                agent,
                Prepared_Data_Real["it_per_epoch"],
                Prepared_Data_Real["bc_train_dataloader"],
                Prepared_Data_Real["bc_validation_dataloader"],
                L,
                epoch,
                sim_real_ratio,
            )
            real_metrics = {
                ...
                "loss/train_kl_real": loss_train_dict_real["train_kl"]
                * args["kl_weight"],
        ...
        elif (epoch + 1) % eval_freq == 0 and (
            (current_rank > 1 and (epoch + 1) >= 100)
            or (current_rank == 1 and (epoch + 1) >= 500)
        ):
...
def main(args):
    ...
    wandb.init(
        project="hand-teleop-adr-rank", name=os.path.basename(log_dir), config=args
    )
