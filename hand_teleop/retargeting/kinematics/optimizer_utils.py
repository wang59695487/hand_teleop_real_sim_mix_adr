...
def add_dummy_free_joint(robot_builder: sapien.ArticulationBuilder,
                         joint_indicator=(True, True, True, True, True, True),
                         translation_range=(-1, 1), rotation_range=(-np.pi, np.pi)):
    ...
    joint_name = [f"dummy_{name}_translation_joint" for name in "xyz"] + [f"dummy_{name}_rotation_joint" for name in
                                                                          "xyz"]
