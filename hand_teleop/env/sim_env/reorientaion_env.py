...
class PourBoxEnv(BaseSimulationEnv):
    ...
    def generate_random_object_pose(self, randomness_scale):
        ...
        pos_x = random.uniform(-0.15, 0.05) * randomness_scale
        pos_y = random.uniform(-0.18, -0.08) * randomness_scale
        position = np.array([pos_x, pos_y, 0.075/2])
        # Randomize orientation
        euler_z_1 = random.uniform(np.deg2rad(0), np.deg2rad(300))
        orientation_z1 = transforms3d.euler.euler2quat(0, 0, euler_z_1)
        orientation_y = transforms3d.euler.euler2quat(0, np.deg2rad(90), 0)
        orientation = transforms3d.quaternions.qmult(
            orientation_y, orientation_z1)
        euler_z_2 = random.uniform(np.deg2rad(60), np.deg2rad(120))
        orientation_z2 = transforms3d.euler.euler2quat(0, 0, euler_z_2)
        orientation = transforms3d.quaternions.qmult(
            orientation_z2, orientation)
