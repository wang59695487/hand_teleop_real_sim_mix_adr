...
class BaseSimulationEnv(object):
    ...
    def load_partnet_obj(self, partnet_id, scale = 0.1, material_spec=[0,0,0], density = 100, fix_root = False):
        ...
        material = self.scene.create_physical_material(*material_spec)

        config = {'material': material, 'density': density}
        part_object = loader.load(urdf, config=config)
        return part_object
