...
class MANORobotHand:
    def __init__(self, scene: sapien.Scene, renderer: sapien.VulkanRenderer, init_joint_pos: np.ndarray,
                 control_interval: float = 0.01, hand_mode="right_hand", free_root=True, position_control=True, scale=1):
        # Visual material
        ...
        self.robot = create_mano_full_dof_robot_hand(scene, renderer, init_joint_pos, free_root=free_root,
                                                     ...
                                                     hand_mode=self.hand_mode, scale=scale)
...
def create_mano_full_dof_robot_hand(scene: sapien.Scene, renderer: sapien.VulkanRenderer, joint_pos: np.ndarray,
                                    finger_viz_mat_kwargs: Dict, tip_viz_mat_kwargs: Dict, hand_mode="right_hand",
...
def main():
    ...
    hand_vertex_index = np.array(hand_info[f'{hand_mode}_hand_verts_idx'])
    if hand_mode == 'right_hand':
    ...
    mano_robot = MANORobotHand(scene, renderer, joint_pos, free_root=True, control_interval=0.1, hand_mode=hand_mode)
    faces = np.load("hand_teleop/kinematics/faces.npy")
    # if hand_mode == 'left':
    #     faces = faces[:, ::-1]
    ...
    while not viewer.closed:
        ...
        mean_pose = smplx_model.right_hand_mean if hand_mode == "right_hand" else smplx_model.left_hand_mean
