...
class DataPlayer:
    def __init__(
        ...
        if zero_joint_pos is not None:
            ...
            self.human_robot_hand = MANORobotHand(
                env.scene,
                env.renderer,
        ...
        scene_actor2id = {
            actor.get_name(): actor.get_id() for actor in self.scene.get_all_actors()
        }
        ...
        for key, value in meta_actor2id.items():
            if key not in scene_actor2id:
                print(
                    f"Demonstration actor {key} not exists in the scene. Will skip it."
                )
            else:
                meta2scene_actor[value] = scene_actor2id[key]

        # Generate articulation id mapping
        all_articulation_root = [
            robot.get_links()[0] for robot in self.scene.get_all_articulations()
        ]
        scene_articulation2id = {
            actor.get_name(): actor.get_id() for actor in all_articulation_root
        }
        scene_articulation2dof = {
            r.get_links()[0].get_name(): r.dof
            for r in self.scene.get_all_articulations()
        }
        ...
        for key, value in meta_articulation2id.items():
            if key not in scene_articulation2id:
                print(
                    f"Recorded articulation {key} not exists in the scene. Will skip it."
                )
            else:
    ...
    def get_sim_data(self, item) -> Dict[str, Any]:
        ...
        scene_actor_data = {
            self.meta2scene_actor[key]: value
            for key, value in actor_data.items()
            if key in self.meta2scene_actor
        }
        scene_drive_data = {
            self.meta2scene_articulation[key]: value
            for key, value in drive_data.items()
            if key in self.meta2scene_articulation
        }
        scene_articulation_data = {
            self.meta2scene_articulation[key]: value
            for key, value in articulation_data.items()
            if key in self.meta2scene_articulation
        }
        return dict(
            actor=scene_actor_data,
            articulation_drive=scene_drive_data,
            articulation=scene_articulation_data,
        )

    @staticmethod
    def collect_env_state(
        actors: List[sapien.Actor], articulations: List[sapien.Articulation] = []
    ):
        ...
        for articulation in articulations:
            ...
            articulation_data = dict(
                qvel=qvel, qpos=qpos, pose=np.concatenate([pose.p, pose.q])
            )
            data["articulation"][links[0].get_id()] = articulation_data
        return data

    def get_finger_tip_retargeting_result(
        self,
        human_robot_hand: MANORobotHand,
...
class RelocateObjectEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(
        self,
        retargeting: Optional[PositionRetargeting] = None,
        method="tip_middle",
        indices=None,
    ):
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            if use_human_hand:
                ...
                if method == "tip_middle":
                    ...
                    qpos = self.get_finger_tip_retargeting_result(
                        self.human_robot_hand,
                        retargeting,
                        indices,
                        use_root_local_pose=use_local_pose,
            ...
            if i >= 1:
                baked_data["action"].append(
                    self.compute_action_from_states(
                        baked_data["robot_qpos"][i - 1],
                        qpos,
...
class FlipMugEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(
        ...
        baked_data = dict(
            obs=[], robot_qpos=[], state=[], action=[], robot_qvel=[], ee_pose=[]
        )
        manipulated_object = self.env.manipulated_object
        use_local_pose = False
        ...
        baked_data["init_mug_pose"] = np.concatenate(
            [manipulated_object_pose.p, manipulated_object_pose.q]
        )
        self.scene.step()
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            if self.env.robot_name == "mano":
                ...
                baked_data["action"].append(
                    self.human_robot_hand.robot.get_drive_target()
                )
...
class TableDoorEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(
        ...
        baked_data["init_door_pose"] = np.concatenate(
            [table_door_pose.p, table_door_pose.q]
        )
        self.scene.step()
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            baked_data["state"].append(
                self.collect_env_state(actors=[], articulations=[table_door])
            )
...
class LaptopEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            # Environment state
            baked_data["state"].append(
                self.collect_env_state(actors=[], articulations=[laptop])
            )
...
class PenDrawEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            # Environment state
            baked_data["state"].append(
                self.collect_env_state(actors=[], articulations=[pen])
            )
