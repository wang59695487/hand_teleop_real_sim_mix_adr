...
class DataPlayer:
    def __init__(self, meta_data: Dict[str, Any], data: Dict[str, Any], env: BaseRLEnv,
        ...
        scene_actor2id = {actor.get_name(): actor.get_id()
        ...
        all_articulation_root = [robot.get_links()[0]
                                 for robot in self.scene.get_all_articulations()]
        scene_articulation2id = {actor.get_name(): actor.get_id()
                                 for actor in all_articulation_root}
        scene_articulation2dof = {r.get_links()[0].get_name(
        ): r.dof for r in self.scene.get_all_articulations()}
    ...
    def collect_env_state(actors: List[sapien.Actor], articulations: List[sapien.Articulation] = []):
        ...
        for actor in actors:
            ...
            pose = actor.get_pose()
            actor_data = dict(velocity=v, angular_velocity=w,
    ...
    def compute_action_from_states(self, robot_qpos_prev, robot_qpos, is_contact: bool):
        ...
        target_position = np.clip(
            (target_position - limit[:, 0]) / (limit[:, 1] - limit[:, 0]) * pos_gain - 1, -1, 1)
    ...
    def bake_demonstration(self, retargeting: Optional[PositionRetargeting] = None, method="tip_middle", indices=None):
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            if i >= 2:
                ...
                finger_qvel = (
                    baked_data["action"][-1][6:] - baked_data["action"][-2][6:]) / duration
...
class FlipMugEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(self, retargeting: Optional[PositionRetargeting] = None, method="tip_middle", indices=None):
        ...
        baked_data = dict(obs=[], robot_qpos=[], state=[],
                          action=[], robot_qvel=[], ee_pose=[])
        manipulated_object = self.env.manipulated_object
        use_local_pose = False
...
class TableDoorEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(self, retargeting: Optional[PositionRetargeting] = None, method="tip_middle", indices=None):
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            # Environment state
            baked_data["state"].append(self.collect_env_state(
                actors=[], articulations=[table_door]))
...
def bake_demonstration_allegro_test(retarget=False):
    ...
    if task_name == 'pick_place':
        ...
        player = HammerEnvPlayer(meta_data, data, env,
    ...
    world_to_robot = transforms3d.affines.compose(-np.matmul(
        rotation_matrix.T, robot_pose.p), rotation_matrix.T, np.ones(3))
...
def bake_visual_real_demonstration_test(retarget=False, index=0):
    ...
    if retarget:
        ...
        record_root = "./real/raw_data/pouring_extend"
