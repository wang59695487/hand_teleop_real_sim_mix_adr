...
def generate_free_robot_hand_info() -> Dict[str, FreeRobotInfo]:
    ...
    allegro_hand_free_info = FreeRobotInfo(path="robot/allegro_hand_description/allegro_hand_free.urdf", dof=22,
                                           palm_name="palm_center")
    allegro_hand_free_left_info = FreeRobotInfo(path="robot/allegro_hand_description/allegro_hand_free_left.urdf", dof=22,
                                           palm_name="palm_center")
    ...
    info_dict = dict(shadow_hand_free=shadow_hand_free_info,
                     ...
                     allegro_hand_free_left=allegro_hand_free_left_info,
...
def generate_arm_robot_hand_info() -> Dict[str, ArmRobotInfo]:
    ...
    allegro_hand_xarm6 = ArmRobotInfo(path=str(xarm_path / "xarm6_allegro.urdf"), hand_dof=16, arm_dof=6,
                                      palm_name="palm", arm_init_qpos=[0, 0, 0, 0, -np.pi / 2, 0],
    ...
    allegro_hand_xarm6_wrist_mounted_face_down = ArmRobotInfo(
        ...
        hand_dof=16, arm_dof=6, palm_name="palm", arm_init_qpos=[0, 0, 0, 0, 0, -np.pi / 2],
        root_offset=[0.00, 0, 0])
    allegro_hand_xarm6_wrist_mounted_face_down_left = ArmRobotInfo(
        path="robot/xarm6_description/xarm6_allegro_wrist_mounted_rotate_left.urdf",
        hand_dof=16, arm_dof=6, palm_name="palm", arm_init_qpos=[0, 0, 0, 0, 0, -np.pi / 2],
        root_offset=[0.00, 0, 0])        
    allegro_hand_xarm6_wrist_mounted_face_front = ArmRobotInfo(
        path="robot/xarm6_description/xarm6_allegro_wrist_mounted_rotate.urdf",
        hand_dof=16, arm_dof=6, palm_name="palm", arm_init_qpos=[0, 0, 0, np.pi, np.pi / 2, np.pi],
        root_offset=[0.00, 0, 0])
    allegro_hand_xarm6_wrist_mounted_face_front_left = ArmRobotInfo(
        path="robot/xarm6_description/xarm6_allegro_wrist_mounted_rotate_left.urdf",
        hand_dof=16, arm_dof=6, palm_name="palm", arm_init_qpos=[0, 0, 0, np.pi, np.pi / 2, 0],
        root_offset=[0.00, 0, 0])
    xarm6_allegro_modified_finger = ArmRobotInfo(
        path="robot/xarm6_description/xarm6_allegro_modified_finger.urdf",
        hand_dof=16, arm_dof=6, palm_name="palm", arm_init_qpos=[0, 0, 0, 0, 0, -np.pi / 2],
        root_offset=[0.00, 0, 0])
    ...
    info_dict = dict(
        ...
        allegro_hand_xarm6_wrist_mounted_face_down_left=allegro_hand_xarm6_wrist_mounted_face_down_left,
        allegro_hand_xarm6_wrist_mounted_face_front_left=allegro_hand_xarm6_wrist_mounted_face_front_left,
        xarm6_allegro_wrist_mounted_rotate = allegro_hand_xarm6_wrist_mounted_face_down,
        xarm6_allegro_modified_finger = xarm6_allegro_modified_finger,
    )
    return info_dict
...
def load_robot(scene: sapien.Scene, robot_name, disable_self_collision=True) -> sapien.Articulation:
    ...
    mat = scene.engine.create_physical_material(30, 20, 0.01)
