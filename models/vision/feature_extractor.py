...
def train_image_encoder(features):
    ...
    optimizer = optim.AdamW(
        params=params, lr=learning_rate, weight_decay=0.001)
    ...
    for t in range(epochs):
        ...
        print("Epoch: {}    Loss: {}".format(
            t, losses/(norm_avg*len(features))))
...
def train_state_decoder(robot_states):
    ...
    for t in range(epochs):
        ...
        optimizer.step()
        print("Epoch: {}    Loss: {}".format(
            t, losses/(norm_avg*len(robot_states))))
...
class ClassToken(nn.Module):
    ...
    def forward(self, tokens):
        if self.cls_token_idx == 0:
            img_feats = tokens["img_feats"][:,
                                            self.cls_token_idx][..., None, None]
            return img_feats
        else:
...
def generate_feature_extraction_model(backbone_type):
    ...
    if "r3m" in backbone_type:
        from r3m import load_r3m
        model = load_r3m("resnet50")  # resnet18, resnet34

    elif "mvp" in backbone_type:
        import mvp
...
def concate_features(features, stack_robot_qpos, stack_frames=True):
    ...
    if stack_frames:
        for i in range(len(features)):
            if i == 0:
                concatenated_obs.append(np.concatenate(
                    (features[i], features[i], features[i], features[i])))
                concatenated_robot_qpos.append(np.concatenate(
                    (stack_robot_qpos[i], stack_robot_qpos[i], stack_robot_qpos[i], stack_robot_qpos[i])))
            elif i == 1:
                concatenated_obs.append(np.concatenate(
                    (features[i-1], features[i], features[i], features[i])))
                concatenated_robot_qpos.append(np.concatenate(
                    (stack_robot_qpos[i-1], stack_robot_qpos[i], stack_robot_qpos[i], stack_robot_qpos[i])))
            elif i == 2:
                concatenated_obs.append(np.concatenate(
                    (features[i-2], features[i-1], features[i], features[i])))
                concatenated_robot_qpos.append(np.concatenate(
                    (stack_robot_qpos[i-2], stack_robot_qpos[i-1], stack_robot_qpos[i], stack_robot_qpos[i])))
            else:
                concatenated_obs.append(np.concatenate(
                    (features[i-3], features[i-2], features[i-1], features[i])))
                concatenated_robot_qpos.append(np.concatenate(
                    (stack_robot_qpos[i-3], stack_robot_qpos[i-2], stack_robot_qpos[i-1], stack_robot_qpos[i])))
    # else:
...
if __name__ == "__main__":
    ...
    visual_dataset = generate_features(
        visual_baked, backbone_type="ResNet34", stack_frames=True, encode=True)
